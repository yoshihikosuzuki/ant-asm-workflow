...
source ../../../config.sh
set -eu
module load ${_BCFTOOLS} ${_BEDTOOLS} ${_SAMTOOLS}
set -x

REF=contigs.fasta
IN_BAM=contigs.hifi.winnowmap.sorted.bam
IN_VCF=contigs.hifi.deepvariant.vcf
...
# Extract homozygous variants (which are deemed misassemblies)
